/*
  ...
  1. New Tables
    - Enhanced `service_calls` table with all required fields
    - `service_call_equipment` table for equipment received/delivered tracking
    - `service_call_feedback` table for customer feedback
    - `service_call_invoices` table for invoice management
    - `service_call_status_history` table for status tracking
  ...
  3. Features
    - Complete service call workflow
    - Equipment tracking
    - Time tracking
    - Feedback system
    - Invoice integration
...
CREATE TABLE service_calls (
  ...
  job_id text UNIQUE NOT NULL DEFAULT 'JOB-' || to_char(now(), 'YYYYMMDD') || '-' || LPAD(nextval('job_id_seq')::text, 4, '0'),
...
-- Add updated_by column to service_calls for tracking who made changes
ALTER TABLE service_calls ADD COLUMN updated_by uuid REFERENCES users(id) ON DELETE SET NULL;
...
-- Create trigger for job ID generation
