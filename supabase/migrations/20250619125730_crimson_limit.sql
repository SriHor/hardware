/*
  # Staff Management System with Designations and Permissions

  1. New Tables
    - `designations` - Job designations with department and level hierarchy
  ...
  3. Security
    ...
    - Admin-only access for designation management
...
DO $$
BEGIN
  -- Enable RLS on designations if not already enabled
  IF NOT EXISTS (
    SELECT 1 FROM pg_class c 
    JOIN pg_namespace n ON n.oid = c.relnamespace 
    WHERE c.relname = 'designations' AND n.nspname = 'public' AND c.relrowsecurity = true
  ) THEN
  ...
  -- Enable RLS on user_permissions_override if not already enabled
  IF NOT EXISTS (
    SELECT 1 FROM pg_class c 
    JOIN pg_namespace n ON n.oid = c.relnamespace 
    WHERE c.relname = 'user_permissions_override' AND n.nspname = 'public' AND c.relrowsecurity = true
  ) THEN
...
BEGIN
  ...
  -- User permissions override policies
